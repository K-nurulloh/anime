...
fetch("products.json")
  ...
  .then(products => {
    ...
    // Thumbnail click
    const mainImg = document.getElementById("main-img");
    document.querySelectorAll("#thumbs img").forEach(t => t.onclick = () => mainImg.src = t.src);
    ...
    document.getElementById("add-cart").onclick = () => {
      ...
      if(!currentUser){
        alert("Savatga qo‘shish uchun avval kirish qiling!");
        return;
      }

      // addToSaved funksiyasini ishlatamiz
      product.qty = qty;
      addToSaved(product);
